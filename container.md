...
* LinkedHashMap：
（1）LinkedHashMap实现了Map接口。（2）LinkedHashMap是有序的。（3）LinkedHashMap底层基于HashMap实现。（4）LinkedHashMap是非线程安全的。（5）LinkedHashMap适用于需要快速删除插入和保证顺序的场景。
* TreeMap
（1）TreeMap实现了NavigableMap接口，Cloneable接口和Serializable接口。（2）TreeMap底层依赖红黑树实现排序，适用于需要排序统计的场景。（3）TreeMap的key不允许为null。